...
exports.init = init;
exports.updateInstanceUrl = updateInstanceUrl;
...
function updateInstanceUrl(url) {
  instanceUrl = url;
}

function init(key, url, cacheAge) {
  token = key;
  instanceUrl = url;
...
_axios["default"].interceptors.response.use(function (response) {
  ...
  setTimeout(function () {
    return emitter.emit(_types.events.DISMISS_LOADER);
  }, 100);
  ...
  if (error.response) {
    ...
    if (status == _httpStatusCodes.StatusCodes.UNAUTHORIZED) {
      emitter.emit(_types.events.UNAUTHORIZED);
    }
...
var axiosCache = (0, _axiosCacheAdapter.setupCache)({
...
var api = /*#__PURE__*/function () {
  var _ref2 = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(customConfig) {
    var config;
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            ...
            if (instanceUrl) config.baseURL = instanceUrl.startsWith('http') ? instanceUrl : "https://".concat(instanceUrl, ".hotwax.io/api/");
            ...
            if (!config.headers) config.headers = _objectSpread(_objectSpread({}, _axios["default"].defaults.headers.common), config.headers);
            emitter.emit(_types.events.QUEUE_TASK, {
          ...
          case 8:
            return _context2.abrupt("return", (0, _axios["default"])(config));
